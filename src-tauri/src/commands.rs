...
pub async fn set_parental_pin(state: State<'_, AppState>, pin: String) -> Result<(), AppError> {
    ...
    if !pin.chars().all(|c| c.is_ascii_digit()) {
        return Err(AppError::InvalidInput("PIN must contain only numbers".to_string()));
    }
    ...
    let db = state.db.lock().await;
    crate::db::operations::set_setting(&db, "parental_pin_hash", &password_hash)?;
...
pub async fn verify_parental_pin(state: State<'_, AppState>, pin: String) -> Result<bool, AppError> {
    ...
    // Get the stored hash
    let hash = match crate::db::operations::get_setting(&db, "parental_pin_hash")? {
        Some(h) => h,
...
pub async fn reset_parental_pin(state: State<'_, AppState>) -> Result<(), AppError> {
    ...
    // Delete the PIN hash
    crate::db::operations::delete_setting(&db, "parental_pin_hash")?;

    // Also disable parental controls
    crate::db::operations::set_setting(&db, "parental_enabled", "false")?;
...
pub async fn get_blocked_channels(state: State<'_, AppState>) -> Result<Vec<i64>, AppError> {
    ...
    let json_str = match crate::db::operations::get_setting(&db, "parental_blocked_channels")? {
        Some(s) => s,
...
pub async fn set_blocked_channels(state: State<'_, AppState>, channel_ids: Vec<i64>) -> Result<(), AppError> {
    ...
    crate::db::operations::set_setting(&db, "parental_blocked_channels", &json_str)?;
...
pub async fn get_parental_settings(state: State<'_, AppState>) -> Result<serde_json::Value, AppError> {
    ...
    let settings = crate::db::operations::get_multiple_settings(
        &db,
        &[
            "parental_enabled",
            "parental_pin_hash",
